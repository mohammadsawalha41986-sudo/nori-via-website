...
export const NEW_TOOLS = [
  ...
  },
  {
    ...
    order: 11,
    featured: false,
    nameEn: 'Gross Profit Calculator',
    nameAr: 'حاسبة الربح الإجمالي',
    summaryEn: 'Gross profit in money and as a margin, calculated from verified revenue and direct cost.',
    summaryAr: 'الربح الإجمالي بالمال وكنسبة هامش، محسوباً من الإيراد والتكلفة المباشرة الموثقين.',
    descriptionEn: 'Enter revenue and the direct cost of the sales behind it. The calculator separates gross profit in money from gross margin as a percentage, so volume and ratio can be read together.',
    descriptionAr: 'أدخل الإيراد والتكلفة المباشرة للمبيعات التي تقف خلفه. تفصل الحاسبة الربح الإجمالي بالمال عن هامش الربح كنسبة، حتى يمكن قراءة الحجم والنسبة معاً.',
    purposeEn: 'To read the money retained after direct cost alongside the margin percentage.',
    purposeAr: 'لقراءة المال المتبقي بعد التكلفة المباشرة إلى جانب نسبة الهامش.',
    config: {
      currency: '',
      inputs: [
        n('revenue', 'Revenue', 'الإيراد', 100000),
        n('directCost', 'Direct cost of sales', 'التكلفة المباشرة للمبيعات', 32000),
      ],
      outputs: [
        out('grossProfit', 'Gross profit', 'الربح الإجمالي', 'revenue - directCost', 'currency', 0, true),
        out('grossMargin', 'Gross margin', 'هامش الربح الإجمالي', 'revenue > 0 ? (revenue - directCost) / revenue * 100 : 0', 'percent', 1),
        out('directCostPct', 'Direct cost %', 'نسبة التكلفة المباشرة', 'revenue > 0 ? directCost / revenue * 100 : 0', 'percent', 1),
      ],
      notesEn: 'Use the same period and scope for both inputs. Gross profit does not include fixed operating costs.',
      notesAr: 'استخدم الفترة والنطاق نفسيهما للمدخلين. لا يشمل الربح الإجمالي التكاليف التشغيلية الثابتة.',
    },
  },
  {
    slug: 'average-check-calculator',
    order: 12,
    featured: false,
    nameEn: 'Average Check Calculator',
    nameAr: 'حاسبة متوسط الفاتورة',
    summaryEn: 'Average check and item attachment from sales, transactions and items sold for the same period.',
    summaryAr: 'متوسط الفاتورة ومعدل إرفاق الأصناف من المبيعات والمعاملات والأصناف المباعة للفترة نفسها.',
    descriptionEn: 'Average check becomes useful when read with transaction volume and items per transaction. This calculator keeps those measures together and makes the period basis explicit.',
    descriptionAr: 'يصبح متوسط الفاتورة مفيداً عند قراءته مع حجم المعاملات وعدد الأصناف لكل معاملة. تجمع هذه الحاسبة تلك المقاييس وتجعل أساس الفترة واضحاً.',
    purposeEn: 'To separate growth from more transactions from growth in the value of each transaction.',
    purposeAr: 'لفصل النمو الناتج عن زيادة المعاملات عن النمو في قيمة كل معاملة.',
    config: {
      currency: '',
      inputs: [n('revenue', 'Revenue', 'الإيراد', 250000), n('transactions', 'Transactions', 'عدد المعاملات', 4200, '', '', 1), n('itemsSold', 'Items sold', 'عدد الأصناف المباعة', 7900)],
      outputs: [
        out('averageCheck', 'Average check', 'متوسط الفاتورة', 'transactions > 0 ? revenue / transactions : 0', 'currency', 2, true),
        out('itemsPerTransaction', 'Items per transaction', 'الأصناف لكل معاملة', 'transactions > 0 ? itemsSold / transactions : 0', 'number', 2),
      ],
      notesEn: 'Keep refunds, discounts, taxes and transaction counts on the same reporting basis.',
      notesAr: 'وحّد أساس التقارير للمبالغ المستردة والخصومات والضرائب وعدد المعاملات.',
    },
  },
  {
    slug: 'seat-productivity-calculator',
    order: 13,
    featured: false,
    nameEn: 'Seat Productivity Calculator',
    nameAr: 'حاسبة إنتاجية المقاعد',
    summaryEn: 'Seat turnover and revenue per available seat for a defined trading period.',
    summaryAr: 'دوران المقاعد والإيراد لكل مقعد متاح خلال فترة تشغيل محددة.',
    descriptionEn: 'Enter covers, seats, revenue and trading days for the same period. The outputs show how often capacity turns and how much revenue each available seat produces.',
    descriptionAr: 'أدخل عدد الضيوف والمقاعد والإيراد وأيام التشغيل للفترة نفسها. توضح النتائج عدد مرات دوران الطاقة والإيراد الذي ينتجه كل مقعد متاح.',
    purposeEn: 'To distinguish a demand problem from a capacity-use problem.',
    purposeAr: 'للتمييز بين مشكلة الطلب ومشكلة استخدام الطاقة الاستيعابية.',
    config: {
      currency: '',
      inputs: [n('covers', 'Covers served', 'عدد الضيوف المخدومين', 6000), n('seats', 'Available seats', 'المقاعد المتاحة', 80, '', '', 1), n('revenue', 'Revenue', 'الإيراد', 360000), n('tradingDays', 'Trading days', 'أيام التشغيل', 30, '', '', 1)],
      outputs: [
        out('seatTurnsPerDay', 'Seat turns per day', 'دوران المقاعد يومياً', 'seats > 0 && tradingDays > 0 ? covers / seats / tradingDays : 0', 'number', 2, true),
        out('revenuePerSeat', 'Revenue per available seat', 'الإيراد لكل مقعد متاح', 'seats > 0 ? revenue / seats : 0', 'currency', 2),
        out('revenuePerSeatDay', 'Revenue per seat per day', 'الإيراد لكل مقعد يومياً', 'seats > 0 && tradingDays > 0 ? revenue / seats / tradingDays : 0', 'currency', 2),
      ],
      notesEn: 'Compare like-for-like dayparts and formats; delivery revenue should be separated when assessing physical seat productivity.',
      notesAr: 'قارن الفترات والصيغ المتشابهة، وافصل إيراد التوصيل عند تقييم إنتاجية المقاعد الفعلية.',
    },
  },
  {
    slug: 'revenue-target-calculator',
    order: 14,
    featured: false,
    nameEn: 'Revenue Target Calculator',
    nameAr: 'حاسبة هدف الإيرادات',
    summaryEn: 'The monthly and daily revenue, transactions and covers required to reach a chosen operating-profit target.',
    summaryAr: 'الإيراد والمعاملات والضيوف شهرياً ويومياً اللازمة للوصول إلى هدف ربح تشغيلي محدد.',
    descriptionEn: 'Start with the profit required, the fixed cost base and the contribution margin. The calculator works backwards to the revenue and daily transaction level needed to support that target.',
    descriptionAr: 'ابدأ بالربح المطلوب وقاعدة التكاليف الثابتة وهامش المساهمة. تعمل الحاسبة عكسياً للوصول إلى الإيراد ومستوى المعاملات اليومية اللازم لدعم الهدف.',
    purposeEn: 'To translate a profit objective into a daily commercial target.',
    purposeAr: 'لتحويل هدف الربح إلى هدف تجاري يومي.',
    config: {
      currency: '',
      inputs: [n('targetProfit', 'Target monthly operating profit', 'هدف الربح التشغيلي الشهري', 50000), n('fixedCosts', 'Monthly fixed costs', 'التكاليف الثابتة الشهرية', 120000), n('contributionMarginPct', 'Contribution margin %', 'نسبة هامش المساهمة', 58), n('averageCheck', 'Average check', 'متوسط الفاتورة', 65, '', '', 1), n('tradingDays', 'Trading days', 'أيام التشغيل', 30, '', '', 1)],
      outputs: [
        out('requiredRevenue', 'Required monthly revenue', 'الإيراد الشهري المطلوب', 'contributionMarginPct > 0 ? (fixedCosts + targetProfit) / (contributionMarginPct / 100) : 0', 'currency', 0, true),
        out('requiredTransactions', 'Required monthly transactions', 'المعاملات الشهرية المطلوبة', 'contributionMarginPct > 0 && averageCheck > 0 ? (fixedCosts + targetProfit) / (contributionMarginPct / 100) / averageCheck : 0', 'number', 0),
        out('requiredDailyRevenue', 'Required daily revenue', 'الإيراد اليومي المطلوب', 'contributionMarginPct > 0 && tradingDays > 0 ? (fixedCosts + targetProfit) / (contributionMarginPct / 100) / tradingDays : 0', 'currency', 0),
        out('requiredDailyTransactions', 'Required daily transactions', 'المعاملات اليومية المطلوبة', 'contributionMarginPct > 0 && averageCheck > 0 && tradingDays > 0 ? (fixedCosts + targetProfit) / (contributionMarginPct / 100) / averageCheck / tradingDays : 0', 'number', 0),
      ],
      notesEn: 'A target is only as reliable as the fixed-cost, margin and average-check assumptions behind it. Test a downside case before adopting it.',
      notesAr: 'موثوقية الهدف لا تتجاوز موثوقية افتراضات التكلفة الثابتة والهامش ومتوسط الفاتورة. اختبر حالة هبوط قبل اعتماده.',
    },
  },
];
